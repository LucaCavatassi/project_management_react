    ...
    return (
        <div className={`${classes.sideBg} ${classes.sideMax} h-full w-1/3`}>
            ...
            {formData.map((data, index) => (
                <p key={index} onClick={() => showDetails(data,index)} className="text-white capitalize py-2 mt-5 font-semibold ms-5 px-2 w-4/5 hover:bg-stone-600 hover:cursor-pointer">{data?.title}</p>
            ))}
        </div>
    )
